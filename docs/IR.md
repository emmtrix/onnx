...
Shape inference propagates string length bounds conservatively: an operation whose output strings are drawn from the string values of its inputs (for example, Identity, Reshape, Concat, Gather, or Where) infers the largest bound among its inputs, provided every input that can carry string values declares a bound. Operations that construct new strings infer a bound only where one can be soundly derived (for example, StringConcat infers the sum of its input bounds), and otherwise leave the output unbounded (for example, casting numeric values to strings). When a value declares a bound and inference derives another, the tighter of the two is kept; for values merged from conditional branches, the larger branch bound is used.
